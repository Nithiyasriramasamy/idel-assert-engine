...
class IotController {
    async getDeviceLogs(req, res) {
        try {
            ...
            const logs = await db_1.prisma.deviceLog.findMany({
                where: { deviceId },
    ...
    async triggerAction(req, res) {
        try {
            ...
            if (action === "UNLOCK") {
                if (!code)
                    return res.status(400).json({ error: "Access passcode required" });
                // Find bookings associated with this device's asset that are active and paid
                const asset = device.asset;
                if (!asset)
                    return res.status(400).json({ error: "No asset linked to this hardware node" });
                const activeBookings = await db_1.prisma.booking.findMany({
                    where: {
                        assetId: asset.id,
